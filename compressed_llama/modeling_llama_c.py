...
# MOVING ALL THE PORTS TO BE DIRECTLY FROM TRANSFORMERS
...
from .configuration_llama_c import LlamaConfig
...
class CustomLLamaMLP(nn.Module):
    ...
    def forward(self, x):
        """
        No tensor parallelism for now. Simple forward pass.
        """
        down_proj = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
...
class _CustomLlamaDecoderLayer(nn.Module):
    ...
    Additionally, it uses a residual_mlp layer to compress the residuals too.
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        # scale down the residuals to the shape of the hidden_states
        residual = self.residual_mlp(residual)
